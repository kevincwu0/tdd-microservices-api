...
class Path(DiagramItem):
    ...
    def arc(self, sweep):
        ...
        self.attrs['d'] += 'a{0} {0} 0 0 {1} {2} {3}'.format(
            ARC_RADIUS, cw, x, y)
        return self
    ...
    def __init__(self, default, *items):
        ...
        for i, item in enumerate(self.items):
            if i < default:
                ...
                assert i > default
                self.down += max(ARC_RADIUS,
                                 VERTICAL_SEPARATION + item.up + item.down)
        if DEBUG:
            self.attrs['data-updown'] = "{0} {1}".format(self.up, self.down)
    ...
    def format(self, x, y, width):
        ...
        for i, item in list(enumerate(above))[::-1]:
            Path(x, y).arc('se').up(distanceFromY -
                                    ARC_RADIUS * 2).arc('wn').addTo(self)
            item.format(x + ARC_RADIUS * 2, y -
                        distanceFromY, innerWidth).addTo(self)
        ...
        self.items[self.default].format(
            x + ARC_RADIUS * 2, y, innerWidth).addTo(self)
        Path(x + ARC_RADIUS * 2 + innerWidth,
             y).right(ARC_RADIUS * 2).addTo(self)
        ...
        for i, item in enumerate(below):
            ...
            Path(x, y).arc('ne').down(distanceFromY -
                                      ARC_RADIUS * 2).arc('ws').addTo(self)
            item.format(x + ARC_RADIUS * 2, y +
                        distanceFromY, innerWidth).addTo(self)
...
class OneOrMore(DiagramItem):
    ...
    def format(self, x, y, width):
        ...
        self.item.format(x + ARC_RADIUS, y, self.width -
                         ARC_RADIUS * 2).addTo(self)
        Path(x + self.width - ARC_RADIUS, y).right(ARC_RADIUS).addTo(self)

        # Draw repeat arc
        distanceFromY = max(ARC_RADIUS*2, self.item.down +
                            VERTICAL_SEPARATION + self.rep.up)
        Path(x + ARC_RADIUS, y).arc('nw').down(distanceFromY - ARC_RADIUS * 2) \
            .arc('ws').addTo(self)
        self.rep.format(x + ARC_RADIUS, y + distanceFromY,
                        self.width - ARC_RADIUS*2).addTo(self)
...
class Start(DiagramItem):
    ...
    def format(self, x, y, _width):
        self.attrs['d'] = 'M {0} {1} v 20 m 10 -20 v 20 m -10 -10 h 20.5'.format(
            x, y - 10)
        return self
...
class Terminal(DiagramItem):
    ...
    def format(self, x, y, width):
        ...
        DiagramItem('text', {'x': x + width / 2,
                             'y': y + 4}, self.text).addTo(self)
        return self
...
class Comment(DiagramItem):
    ...
    def format(self, x, y, width):
        ...
        DiagramItem('text', {'x': x + width / 2, 'y': y + 5,
                             'class': 'comment'}, self.text).addTo(self)
        return self
